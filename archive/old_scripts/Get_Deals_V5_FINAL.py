"""
═══════════════════════════════════════════════════════════════════════════════
Calgary Grocery Hub - Get Deals V5 (Calibration Architecture)
═══════════════════════════════════════════════════════════════════════════════

VERSION: 5.0 - Calibration Architecture (Dec 2025)
AUTHOR: Built with Claude (Anthropic)

ARCHITECTURE PHILOSOPHY:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
API = TRUTH
  ├─ Flipp API provides complete dataset (1,400+ items)
  ├─ Clean, structured, reliable data
  └─ This is your PRIMARY source

PDF = CALIBRATION  
  ├─ Extracts SIGNALS only (not every item)
  ├─ Member pricing indicators
  ├─ Featured/prominent items
  ├─ Price spot-checks for validation
  └─ Enhances API data with context

Gemini 3 Flash = SIGNAL EXTRACTION
  ├─ Focused task: extract 60-100 calibration points
  ├─ NOT doing pixel-perfect OCR (wrong tool for that)
  ├─ IS identifying visual patterns & signals (what it's good at)
  └─ ~$0.03-0.06 per store (vs $0.15+ trying to extract everything)

Claude Sonnet 4 = INTELLIGENT ANALYSIS
  ├─ Receives API data + PDF calibration signals
  ├─ Analyzes deals with full context
  ├─ Knows which items are member exclusive
  └─ This is the intelligence layer you're selling

WORKFLOW:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. Scrape API → 1,400 items (COMPLETE DATASET) ✅
2. Extract PDF signals → 60-100 calibration points ✅
3. Match signals to API → Enhanced dataset ✅
4. Claude analysis → Intelligent recommendations ✅
5. Save enhanced data → Dashboard ready ✅

REQUIREMENTS:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- Python 3.8+
- pip install requests pandas python-dotenv sqlalchemy PyMuPDF pillow google-generativeai anthropic

.env FILE:
ANTHROPIC_API_KEY=your_claude_api_key
GEMINI_API_KEY=your_gemini_api_key (optional, for PDF calibration)

PDF CALIBRATION (Optional):
- Create flyers/ folder
- Add PDFs named like: "Sobeys Jan 2-7.pdf", "Co-op Jan 2-7.pdf"
- System will auto-detect and calibrate

OUTPUTS:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- historical_archive.csv (all-time record with AI analysis)
- current_flyers.csv (this week's deals)
- clean_grocery_data.csv (dashboard file)
- member_exclusive_items.csv (if PDF calibration enabled)
...
# PDF Calibration imports (optional - only loads if PDFs present)
...
def extract_calibration_signals(images, store_name):
    ...
    for page_num, img in enumerate(images, 1):
        ...
        # Progress indicator
        progress_pct = (page_num / len(images)) * 100
        current_time = datetime.now().strftime("%H:%M:%S")
        
        # Calculate ETA
        if page_times:
            avg_page_time = sum(page_times) / len(page_times)
            pages_remaining = len(images) - page_num
        ...
        print(f"         [{current_time}] Page {page_num}/{len(images)} ({progress_pct:.1f}%) - ETA: {eta_str}")
...
Keep responses SHORT. We're calibrating, not extracting everything."""
        ...
        try:
            ...
            print(f"            ✅ API response in {api_elapsed:.1f}s")
            ...
            print(f"            📝 Extracted {signals_found} signals in {page_elapsed:.1f}s")
    ...
    print(f"         ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print(f"         ✅ PDF processing complete in {time_str}")
    print(f"         📊 Results: Member={len(calibration_data['member_items'])}, Featured={len(calibration_data['featured_items'])}, Spots={len(calibration_data['spot_checks'])}")
...
print("=" * 80)
print("🛒 CALGARY GROCERY HUB - API + PDF CALIBRATION v5.0")
print("=" * 80)
...
total_stores = len(selected_flyers)

for store_idx, flyer in enumerate(selected_flyers, 1):
    store_start = time.time()
    current_time = datetime.now().strftime("%H:%M:%S")
    progress_pct = (store_idx / total_stores) * 100
    
    print(f"\n📄 [{current_time}] Store {store_idx}/{total_stores} ({progress_pct:.1f}%): {flyer['merchant']}")
    print(f"   🔄 Fetching items from API...")
    
    items = get_flyer_items(flyer['id'])
    
    store_elapsed = time.time() - store_start
    print(f"   ✅ Found {len(items)} items in {store_elapsed:.1f}s")
...
scrape_minutes = int(scrape_elapsed / 60)
scrape_seconds = int(scrape_elapsed % 60)
time_str = f"{scrape_minutes}m {scrape_seconds}s" if scrape_minutes > 0 else f"{scrape_seconds}s"

print(f"\n{'=' * 80}")
print(f"✅ API SCRAPING COMPLETE in {time_str}")
print(f"   Total items extracted: {len(new_deals)}")
print(f"   Avg items per store: {len(new_deals) / total_stores:.0f}")
print(f"{'=' * 80}")
...
if pdf_calibration_enabled():
    ...
    if store_pdfs:
        ...
        for store_idx, (store, pdf_path) in enumerate(store_pdfs.items(), 1):
            store_start = time.time()
            current_time = datetime.now().strftime("%H:%M:%S")
            progress_pct = (store_idx / len(store_pdfs)) * 100
            
            print(f"\n{'─' * 80}")
            print(f"🏪 [{current_time}] PDF {store_idx}/{len(store_pdfs)} ({progress_pct:.1f}%): {store}")
            ...
            try:
                ...
                print(f"   ✅ Converted {len(images)} pages in {convert_elapsed:.1f}s")
                ...
                store_elapsed = time.time() - store_start
                print(f"\n   ✅ Store complete in {store_elapsed:.1f}s")
                
            except Exception as e:
                print(f"      ❌ Error: {str(e)[:80]}")
                continue
            
            if store_idx < len(store_pdfs):
                print(f"   ⏳ Pausing 1s before next store...")
                time.sleep(1)
        
        pdf_total_elapsed = time.time() - pdf_start_time
        pdf_minutes = int(pdf_total_elapsed / 60)
        pdf_seconds = int(pdf_total_elapsed % 60)
        pdf_time_str = f"{pdf_minutes}m {pdf_seconds}s" if pdf_minutes > 0 else f"{pdf_seconds}s"
        
        print(f"\n{'=' * 80}")
        print(f"✅ ALL PDFs PROCESSED in {pdf_time_str}")
...
# ───────────────────────────────────────────────────────────────────────────
# LOAD HISTORICAL DATA & IDENTIFY NEW ITEMS
# ───────────────────────────────────────────────────────────────────────────
...
if len(items_to_analyze) > 0:
    ...
    print(f"\n✅ No new items to analyze - all {len(items_already_done)} already in archive!")
