...
print(f"   This will take ~1-1.5 hours for {len(items_for_analysis):,} items")
print(f"   Processing in batches of 200 (with trimmed price histories)...")

analyzer = AIQualityAnalyzer()

all_analyses = []
batch_size = 200  # Increased again - prompts are now trimmed to last 10 prices
    ...
    try:
        ...
        if batch_num == 1:
            ...
            if other_pct > 80:
                ...
                print(f"   ✅ Categories look good: {other_pct:.1f}% 'Other', {100-other_pct:.1f}% categorized")
            ...
            if unique_ratings < 3:
                print(f"   ⚠️  WARNING: Only {unique_ratings} unique deal ratings found")
                print(f"   AI may be using defaults")
                response = input(f"   Continue anyway? (y/n): ")
                if response.lower() != 'y':
                    print(f"   Stopped by user.")
                    exit(1)
            else:
                print(f"   ✅ Deal ratings vary: {unique_ratings} different ratings")
            ...
            print(f"   ✅ Subcategories: {subcat_pct:.1f}% have subcategories")
            
            # Check reliability distribution
            reliability = [a.reliability for a in analyses]
            insufficient_pct = (sum(1 for r in reliability if r == "insufficient") / len(reliability)) * 100
            
            print(f"   ℹ️  Reliability: {insufficient_pct:.1f}% insufficient (expected for items with little history)")
            
            print(f"\n   🎯 VALIDATION PASSED! Continuing with remaining {total_batches - 1} batches...")
            print(f"   (This will take approximately {((total_batches - 1) * 2):.0f} more minutes)\n")
